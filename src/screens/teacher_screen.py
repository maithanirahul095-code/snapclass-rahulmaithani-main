...
from src.database.db import create_teacher, check_teacher_exists , teacher_login , get_teacher_subject , get_attendence_for_teacher
...
import pandas as pd
from src.components.dialog_voice_attendence import show_attendence_result
...
def teacher_dashboard():
     ...
     with tab1 :
         ...
         if st.button("Take_Attendence" ,type=type1,width="stretch" , icon=":material/ar_on_you:"):
              st.session_state.current_teacher_tab = "take_attendence"
              st.rerun()
     with tab2 :
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username =st.text_input("Enter Username",placeholder="RahulMaithani" )
...
def teacher_screen_register():
    ...
    with col1 :
        header_dashboard()
    with col2:
        if st.button("Go back to Home" , type = "secondary" , key="loginbackbutton2" , shortcut="control+backspace"):
                     st.session_state["login_type"]= None
                     st.rerun()
    ...
    st.space()
    teacher_username =st.text_input("Enter Username",placeholder="RahulMaithani" )
    teacher_name =st.text_input("Enter name",placeholder="Rahul" )
